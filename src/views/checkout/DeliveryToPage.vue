            ...
            <form @submit.stop.prevent="onSubmit">
                ...
                <template v-if="!store.state.isLogin">
                    <ion-item class="form-field">
                        <ion-input v-bind="customer_name" :label="$t('Name')" label-placement="floating"
                    ...
                    <TelephoneInput v-if="props.isReady" :country_code="country_code.value" :phone_number="phone_number.value" 
                        @onChange="onTelephoneChange"></TelephoneInput>
                    <!--
                    <ion-item class="form-field">
                        <input type="hidden" name="country_code" :value="country_code" />
                        <ion-input v-bind="phone_number" :label="$t('Phone no.')" label-placement="floating"
                            :placeholder="$t('Enter phone no.')"></ion-input>
                    </ion-item>-->
                    <p class="txt-field-error" v-if="props.isReady && errors.phone_number">
                        {{ $t("Phone number is a not valid field") }}
                    </p>

                    <ion-item class="form-field">
                        <ion-input v-bind="email" :label="$t('Email')" label-placement="floating"
                ...
                <ion-item class="form-field area-field" tappable @click="selectArea">
                    ...
                    <ion-label v-if="props.area">
                        <h5>{{ $t("Area") }}</h5>
                        <p>{{ props.area.area_name }}</p>
                    </ion-label>
                    <ion-label v-if="props.city">
                        <h5>{{ $t("City") }}</h5>
                        <p>{{ props.city.city_name }}</p>
                    </ion-label>
                    <ion-label v-if="props.state">
                        <h5>{{ $t("State") }}</h5>
                        <p>{{ props.state.name }}</p>
                    </ion-label>

                    <ion-label v-if="!props.state && !props.city && !props.area">
                        <h5>{{ $t("Location") }}</h5>
                        <p>{{ $t("Set your location") }}</p>
                    </ion-label>
                    <img src="@/assets/icons/chevron-right.svg" class="rotate-ar" slot="end" />
                </ion-item>
                ...
                <div class="type-wrapper">
                    ...
                    <button class="btn-work" type="button" @click="setFieldValue('unit_type', 'office')" :class="{ active: values.unit_type == 'office' }">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                ...
                <p class="txt-field-error" v-if="meta.validated && errors.unit_type">
                    {{ $t('{field} is a required field', { 'field': $t('Unit type') }) }}
                </p>
                ...
                <ion-item v-if="values.unit_type == 'apartment'" class="form-field">
                    <ion-input v-bind="apartment" :label="$t('Apartment')" label-placement="floating"
                        :placeholder="$t('Enter apartment')"></ion-input>
                </ion-item>
                <p class="txt-field-error" v-if="errors.apartment">
                    {{ $t("Apartment is a not valid field") }}
                </p>
                ...
                <ion-item class="form-field">
                    <ion-input v-bind="block" :label="$t('Block number (optional)')" label-placement="floating" 
                        :placeholder="$t('Enter block number')"></ion-input>
                </ion-item>
                <p class="txt-field-error" v-if="errors.block">
                    {{ $t("Block is a not valid field") }}
                </p>

                <ion-item class="form-field">
                    <ion-input v-bind="avenue" :label="$t('Avenue (optional)')" label-placement="floating" 
                        :placeholder="$t('Enter avenue name')"></ion-input>
                </ion-item>
                <p class="txt-field-error" v-if="errors.avenue">
                    {{ $t("Avenue is a not valid field") }}
                </p>

                <ion-item class="form-field">
                    <ion-input v-bind="building" :label="$t('Building name (optional)')" label-placement="floating" 
                        :placeholder="$t('Enter building name')"></ion-input>
                </ion-item>
                ...
                <ion-item class="form-field">
                    <ion-input v-bind="street" :label="$t('Street')" label-placement="floating" 
                        :placeholder="$t('Enter street')"></ion-input>
                </ion-item>
                ...
                <div class="type-wrapper">
                    <button class="btn-leave-at-door" type="button" @click="setFieldValue('delivery_instructions', $t('Leave at door'))" :class="{ active: values.delivery_instructions == 'Leave at door' }">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    <button class="btn-hand-it" type="button" @click="setFieldValue('delivery_instructions', $t('Hand it to me'))" :class="{ active: values.delivery_instructions == 'Hand it to me' }">
                    ...
                    <button class="btn-leave" type="button" @click="setFieldValue('delivery_instructions', $t('Leave with guard'))" :class="{ active: values.delivery_instructions == 'Leave with guard' }">
...
import { IonInput, IonItem, IonLabel, IonButtons, IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonSpinner, modalController } from '@ionic/vue';
...
import AreaPage from '../profile/address/AreaPage.vue';
...
onMounted(() => {
    ...
    trackView('delivery-to-page');
 
    if(store.state.customerAddress && store.state.customerAddress.country_id) {

        setValues(store.state.customerAddress);

        props.area = store.state.customerAddress.area;
        props.city = store.state.customerAddress.city;
        props.state = store.state.customerAddress.state;
        props.country = store.state.customerAddress.country;
     ...
     * if(store.state.customerInfo) {
        setFieldValue('customer_name', store.state.customerInfo.customer_name);
        setFieldValue('phone_number', store.state.customerInfo.phone_number);
        setFieldValue('country_code', store.state.customerInfo.country_code);
        setFieldValue('email', store.state.customerInfo.email);
    }
     */
...
const onSubmit = handleSubmit(values => {
    ...
    store.commit('setCustomerAddress', params);
